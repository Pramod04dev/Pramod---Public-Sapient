...
resource "null_resource" "secondary_subnet_shared_tags" {
  count = "${length(var.secondary_subnet_ids)}"
  provisioner "local-exec" {
    command = "aws --region ${var.region} ec2 create-tags --resource ${var.secondary_subnet_ids[count.index]} --tags Key=kubernetes.io/cluster/${var.cluster_name},Value=shared Key=kubernetes.io/role/internal-elb,Value=1"
  }

  provisioner "local-exec" {
    when = "destroy"
    command = "aws --region ${var.region} ec2 delete-tags --resources ${var.secondary_subnet_ids[count.index]} --tags Key=kubernetes.io/cluster/${var.cluster_name},Value=shared"
  }
}
