...
resource "aws_security_group_rule" "eks_cluster_nodes_alb_ingress" {
  count            = length(var.eks_cluster_nodes_sg_ids)
  security_group_id = var.eks_cluster_nodes_sg_ids[count.index]
  source_security_group_id = aws_security_group.eks_cluster_service_elb_sg.id
  description       = "Allow ALB access to EKS nodes"
  from_port         = 0
  to_port           = 65535
  protocol          = "tcp"
  type              = "ingress"
...
resource "aws_security_group" "eks_cluster_service_elb_sg" {
  name        = "${var.project}-eks-cluster-service-elb-sg-${var.environment}"
  ...
  ingress {
    ...
    to_port          = 443
    protocol         = "tcp"
    ipv6_cidr_blocks = "${var.lb_ipv6_cidr}"
  }
...
resource "aws_iam_role" "eks_cluster_nodes_role" {
  name = "${var.project}-eks-cluster-nodes-role-${var.environment}"
...
resource "aws_iam_policy" "eks_cluster_nodes_policy_dynamodb" {
  name = "${var.project}-eks-cluster-nodes-eks_cluster_nodes_policy_dynamodb-policy-${var.environment}"
  path        = "/"
  description = "${var.project}-eks-cluster-nodes-eks_cluster_nodes_policy_dynamodb-policy-${var.environment}"
...
resource "aws_iam_policy" "eks_cluster_nodes_policy_CloudWatchAgentServerPolicy" {
  name = "${var.project}-eks-cluster-nodes-CloudWatchAgentServerPolicy-policy-${var.environment}"
  path        = "/"
  description = "${var.project}-eks-cluster-nodes-CloudWatchAgentServerPolicy-policy-${var.environment}"
...
resource "aws_iam_policy" "eks_cluster_nodes_policy_podautoscalling" {
  name = "${var.project}-eks-cluster-nodes-podautoscalling-policy-${var.environment}"
  path        = "/"
  description = "${var.project}-eks-cluster-nodes-podautoscalling-policy-${var.environment}"
...
resource "aws_iam_policy" "eks_cluster_nodes_policy_ingresscontroller" {
  name = "${var.project}-eks-cluster-nodes-ingresscontroller-policy-${var.environment}"
  path        = "/"
  description = "${var.project}-eks-cluster-nodes-ingresscontroller-policy-${var.environment}"
...
resource "aws_iam_role_policy_attachment" "eks_cluster_nodes_role_CloudWatchAgentServerPolicy" {
  policy_arn = "${aws_iam_policy.eks_cluster_nodes_policy_CloudWatchAgentServerPolicy.arn}"
  role       = "${aws_iam_role.eks_cluster_nodes_role.name}"
}
